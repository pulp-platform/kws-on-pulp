...
void FullyConnected10(
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      y_tile_size_nof = (_i_nof_load+1 == 1) ? 12 : 12;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 1, 1, 64, 1, 64,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8);
    ...
    W = (uint8_t *) (l1_buffer + 128);
    y = (uint8_t *) (l1_buffer + 72);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 1, 1, 12, 1, 12, 0, 0, 0, 0, 0, 0, 32);
      DMA_copy_y.loc = (l1_buffer + 72);
