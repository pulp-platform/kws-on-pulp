...
if __name__=="__main__":
    template_dir     = sys.argv[1] #"/mnt/ssd_big/Thesis/fscil_tiny/dory/uart_input"
    dory_network_dir = sys.argv[2] #"/mnt/ssd_big/Thesis/dory/application/005/"

    # Edit files in SRC
    src_loc = os.path.join(dory_network_dir,"src")
    files_to_update = ["Addition", "BNReluConvolution", "Linear", "FullyConnected", "Pooling", "main.c"]
    lines_to_remove = ["dory_dma_channel", "dory_dma_free", "pulp.h", "dma_id"]
    string_to_replace = [
        ("dory_dma.h", "thorir_dma.h"),
        ("dory_get_tile.h", "dory.h"),
        ("dory_dma_memcpy_async\(", "thorir_dma("),
    ...
    shutil.copy(os.path.join(template_dir,"thorir_dma.c"), src_loc)
    shutil.copy(os.path.join(template_dir,"dory.h"), inc_loc)
    shutil.copy(os.path.join(template_dir,"thorir_dma.h"), inc_loc)
