...
void BNReluConvolution9(
  ...
  ////////////////////////////
  DMA_copy_k.ext = (uint32_t) l2_W+2484;
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+3588;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (_i_nif_load+1 == 6) ? 36 : 48;
      ...
      x_length_nif_byte = (_i_nif_load+1 == 6)   ? 36 : 48;
      ...
      W_tile_size_nof = (_i_nof_load+1 == 6) ? 36 : 48;
      W_tile_size_nif = (_i_nif_load+1 == 6) ? 1 : 1;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*3*3;
      W_length_nif_byte = (_i_nif_load+1 == 6) ? 1 : 1;
      // transfer of next input tile in double buffering

      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 25, 5, 48, 5, 276,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+2484 + 192*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 24888 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        thorir_dma(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+3588 + 192*_i_nof_load;
    ...
    x_tile_size_nif_exec = (_i_nif_exec+1 == 6) ? 36 : 48;
    x_tile_size_h_exec   = (_i_h_exec+1 == 1)   ? 25 : 25;
    x_tile_size_w_exec   = (_i_w_exec+1 == 1)   ? 5 : 5;
    y_tile_size_nof = (_i_nof_exec+1 == 6) ? 36 : 48;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 6)   ? 36 : 48;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
   ...
   if(iter < (total_tiles-1) && (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec))
   {
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 25, 5, 48, 5, 276, 0, 0, 0, 0, 0, 0, 8);
