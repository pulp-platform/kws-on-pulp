...
void BNReluConvolution10(
  ...
  DMA_copy_x.stride_2d = 1380;
  DMA_copy_x.stride_1d = 276;
  DMA_copy_x.dir = 1;

  DMA_copy_W.hwc_to_chw = 0;
  DMA_copy_W.stride_2d = 276;
  DMA_copy_W.stride_1d = 276;
  DMA_copy_W.dir = 1;

  DMA_copy_y.hwc_to_chw = 0;
  DMA_copy_y.stride_2d = 1380;
  DMA_copy_y.stride_1d = 276;
  DMA_copy_y.dir = 0;
  ...
  ////////////////////////////
  DMA_copy_k.ext = (uint32_t) l2_W+76176;
  DMA_copy_k.loc = (uint32_t) l1_buffer + 29880;
  DMA_copy_k.number_of_2d_copies = 1;
  DMA_copy_k.number_of_1d_copies = 1;
  DMA_copy_k.length_1d_copy = (uint16_t) 144;
  thorir_dma(&DMA_copy_k);
  pi_cl_team_barrier(0);


  DMA_copy_lambda.ext = (uint32_t) l2_W+77280;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 30176;
  DMA_copy_lambda.number_of_2d_copies = 1;
  DMA_copy_lambda.number_of_1d_copies = 1;
  DMA_copy_lambda.length_1d_copy = (uint16_t) 144;
  ...
  DMA_copy_x.length_1d_copy = 276;
  thorir_dma(&DMA_copy_x);
  pi_cl_team_barrier(0);

  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 10000) + 0;
  DMA_copy_W.number_of_2d_copies = 36;
  DMA_copy_W.number_of_1d_copies = 1;
  DMA_copy_W.length_1d_copy = 276;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_x = !db_state_x ? 4416 : 0;
    db_W = !db_state_W ? 9936 : 0;
    db_y = !db_state_y ? 576 : 0;
    db_act = !db_state_W ? 144 : 0;
    exec_db_x = db_state_x ? 4416 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 9936 : 0;
    exec_db_act = db_state_W ? 144 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (_i_nif_load+1 == 1) ? 276 : 276;
      x_tile_size_h   = (_i_h_load+1 == 4)   ? 1 : 8;
      x_tile_size_w   = (_i_w_load+1 == 3)   ? 1 : 2;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*8/8;
      x_length_nif_byte = (_i_nif_load+1 == 1)   ? 276 : 276;
      ...
      W_tile_size_nof = (_i_nof_load+1 == 8) ? 24 : 36;
      W_tile_size_nif = (_i_nif_load+1 == 1) ? 276 : 276;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*8*1*1/8;
      W_length_nif_byte = (_i_nif_load+1 == 1) ? 276 : 276;
      // transfer of next input tile in double buffering

      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 2, 276, 5, 276,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 36, 1*1, 276, 1*1, 276, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 10000) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        thorir_dma(&DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+76176 + 144*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 29880 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        thorir_dma(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+77280 + 144*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 30176 + db_act;
    ...
    k = (int32_t *) (l1_buffer + 29880 + exec_db_act);
    lambda = (int32_t *) (l1_buffer + 30176 + exec_db_act);
    W = (uint8_t *) (l1_buffer + 10000 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 8840 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 276 : 276;
    x_tile_size_h_exec   = (_i_h_exec+1 == 4)   ? 1 : 8;
    x_tile_size_w_exec   = (_i_w_exec+1 == 3)   ? 1 : 2;
    y_tile_size_nof = (_i_nof_exec+1 == 8) ? 24 : 36;
    y_tile_size_h   = (_i_h_exec+1 == 4)   ? 1 : 8;
    y_tile_size_w   = (_i_w_exec+1 == 3)   ? 1 : 2;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
    y_length_nof_byte = (_i_nof_exec+1 == 8)   ? 24 : 36;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
   ...
   if(iter < (total_tiles-1) && (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec))
   {
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 2, 36, 5, 276, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 8840) + db_y;
