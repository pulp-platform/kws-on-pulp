...
void execute_layer_fork(void *args) {
  ...
  switch (layer_args->layer_id)
  {
    ...
    case 8:
      ...
      break;
    case 9:
      pi_cl_team_fork(NUM_CORES, (void *)BNReluConvolution9, args);
      break;
    case 10:
      pi_cl_team_fork(NUM_CORES, (void *)BNReluConvolution10, args);
      break;
    case 11:
      pi_cl_team_fork(NUM_CORES, (void *)ReluPooling11, args);
      break;
    case 12:
      pi_cl_team_fork(NUM_CORES, (void *)FullyConnected12, args);
      break;
  }
...
void network_run(void *l2_buffer, size_t l2_buffer_size, void *l2_final_output, void ** L3_final_weights_curr, int exec)
{
  ...
  #ifdef VERBOSE
  print_perf("Final", cycle_network_execution, 50545812);
  #endif

  // ODDA
  // 11 layers with weights have been processed before FC layer  
...
/* ---------------------------------- */
  ...
  // ODDA
  // int n_inf_layers = 13; // Inference
  int n_inf_layers = 12; // Training
...
*/
    ...
    if (L3_output_layers[i]==1) {
      ...
      checksum(i + 1 < 13 ? "L2 output" : "final output",
  ...
  //memcpy(L2_output, l2_final_output, activations_out_size[12]); // BUGGY!
  for (int i=0; i<activations_out_size[12]; i++)
