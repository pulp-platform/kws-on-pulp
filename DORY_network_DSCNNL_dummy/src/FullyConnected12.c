...
void FullyConnected12(
  ...
  im2col = l1_buffer + 3724;
  uint16_t out_shift = out_shift_in;

  ////////////////////////////
  // First tile transfering //
  ////////////////////////////
  DMA_copy_bias.ext = (uint32_t) l2_W+3312;
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 3676);
  ...
  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 340) + 0;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_x = !db_state_x ? 276 : 0;
    db_W = !db_state_W ? 3312 : 0;
    db_y = !db_state_y ? 48 : 0;
    exec_db_x = 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 3312 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      W_tile_size_nof = (_i_nof_load+1 == 1) ? 12 : 12;
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 12, 1*1, 276, 1*1, 276, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 340) + db_W;
    ...
    b = (uint8_t *) (l1_buffer + 3676 + _i_nof_exec*48);
    W = (uint8_t *) (l1_buffer + 340 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 284 + db_y);
    ...
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 12 : 12;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 1)   ? 48 : 48;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
   ...
   // wait for DMA write/read
     pi_cl_team_barrier(0);
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 1, 1, 12, 1, 12, 0, 0, 0, 0, 0, 0, 32);
      DMA_copy_y.loc = (l1_buffer + 284) + db_y;
