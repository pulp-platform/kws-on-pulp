...
void BNReluConvolution0(
  ...
  DMA_copy_x.stride_2d = 10;
  DMA_copy_x.stride_1d = 1;
  DMA_copy_x.dir = 1;

  DMA_copy_W.hwc_to_chw = 0;
  DMA_copy_W.stride_2d = 40;
  ...
  DMA_copy_x.number_of_2d_copies = 24;
  DMA_copy_x.number_of_1d_copies = 6;
  DMA_copy_x.length_1d_copy = 1;
  thorir_dma(&DMA_copy_x);
  pi_cl_team_barrier(0);

  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 8752) + 0;
  DMA_copy_W.number_of_2d_copies = 264;
  DMA_copy_W.number_of_1d_copies = 40;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_x = !db_state_x ? 144 : 0;
    db_W = !db_state_W ? 10560 : 0;
    db_y = !db_state_y ? 4224 : 0;
    db_act = !db_state_W ? 1056 : 0;
    exec_db_x = db_state_x ? 144 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 10560 : 0;
    exec_db_act = db_state_W ? 1056 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      x_tile_size_h   = (_i_h_load+1 == 4)   ? 6 : 24;
      x_tile_size_w   = (_i_w_load+1 == 3)   ? 3 : 6;
      ...
      W_tile_size_nof = (_i_nof_load+1 == 2) ? 12 : 264;
      W_tile_size_nif = (_i_nif_load+1 == 1) ? 1 : 1;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*8*10*4/8;
      W_length_nif_byte = (_i_nif_load+1 == 1) ? 1 : 1;
      // transfer of next input tile in double buffering

      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 24, 6, 1, 10, 1,  8, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 264, 10*4, 1, 10*4, 1, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 8752) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        thorir_dma(&DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+11040 + 1056*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 29880 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        thorir_dma(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+12144 + 1056*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 32000 + db_act;
    ...
    lambda = (int32_t *) (l1_buffer + 32000 + exec_db_act);
    W = (uint8_t *) (l1_buffer + 8752 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 296 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 1 : 1;
    x_tile_size_h_exec   = (_i_h_exec+1 == 4)   ? 6 : 24;
    x_tile_size_w_exec   = (_i_w_exec+1 == 3)   ? 3 : 6;
    y_tile_size_nof = (_i_nof_exec+1 == 2) ? 12 : 264;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 2)   ? 12 : 264;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
   ...
   if(iter < (total_tiles-1) && (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec))
   {
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 2, 264, 5, 276, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 296) + db_y;
