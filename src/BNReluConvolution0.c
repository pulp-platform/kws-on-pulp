...
void BNReluConvolution0(
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+2816;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 11338;
  ...
  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 8506) + 0;
  DMA_copy_W.number_of_2d_copies = 64;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_W = !db_state_W ? 2560 : 0;
    db_y = !db_state_y ? 8000 : 0;
    db_act = !db_state_W ? 256 : 0;
    exec_db_x = 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 2560 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_W.loc = (l1_buffer + 8506) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        thorir_dma(&DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+2560 + 256*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 11074 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        thorir_dma(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+2816 + 256*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 11338 + db_act;
    ...
    lambda = (int32_t *) (l1_buffer + 11338 + exec_db_act);
    W = (uint8_t *) (l1_buffer + 8506 + exec_db_W);
    ...
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 64 : 64;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 1)   ? 64 : 64;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
   ...
   if(iter < (total_tiles-1) && (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec))
   {
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 25, 5, 64, 5, 64, 0, 0, 0, 0, 0, 0, 8);
