...
void BNReluConvolution4(
  ...
  ////////////////////////////
  DMA_copy_k.ext = (uint32_t) l2_W+4096;
  DMA_copy_k.loc = (uint32_t) l1_buffer + 20120;
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+4352;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 20384;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 64, 1*1, 64, 1*1, 64, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 16016) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        thorir_dma(&DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+4096 + 256*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 20120 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        thorir_dma(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+4352 + 256*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 20384 + db_act;
    ...
    k = (int32_t *) (l1_buffer + 20120 + exec_db_act);
    lambda = (int32_t *) (l1_buffer + 20384 + exec_db_act);
    W = (uint8_t *) (l1_buffer + 16016 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 8008 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 64 : 64;
