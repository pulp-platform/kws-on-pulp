...
void FullyConnected10(
  ...
  im2col = l1_buffer + 2372;
  uint16_t out_shift = out_shift_in;

  ////////////////////////////
  // First tile transfering //
  ////////////////////////////
  DMA_copy_bias.ext = (uint32_t) l2_W+2064;
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 2324);
  ...
  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 236) + 0;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_x = !db_state_x ? 172 : 0;
    db_W = !db_state_W ? 2064 : 0;
    db_y = !db_state_y ? 48 : 0;
    exec_db_x = 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 2064 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 12, 1*1, 172, 1*1, 172, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 236) + db_W;
    ...
    b = (uint8_t *) (l1_buffer + 2324 + _i_nof_exec*48);
    W = (uint8_t *) (l1_buffer + 236 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 180 + db_y);
   ...
   // wait for DMA write/read
     pi_cl_team_barrier(0);
      ...
      DMA_copy_y.loc = (l1_buffer + 180) + db_y;
