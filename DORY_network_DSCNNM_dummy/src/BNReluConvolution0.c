...
void BNReluConvolution0(
  ...
  ////////////////////////////
  DMA_copy_k.ext = (uint32_t) l2_W+6880;
  DMA_copy_k.loc = (uint32_t) l1_buffer + 28894;
  DMA_copy_k.number_of_2d_copies = 1;
  DMA_copy_k.number_of_1d_copies = 1;
  DMA_copy_k.length_1d_copy = (uint16_t) 688;
  thorir_dma(&DMA_copy_k);
  pi_cl_team_barrier(0);


  DMA_copy_lambda.ext = (uint32_t) l2_W+7568;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 29590;
  DMA_copy_lambda.number_of_2d_copies = 1;
  DMA_copy_lambda.number_of_1d_copies = 1;
  DMA_copy_lambda.length_1d_copy = (uint16_t) 688;
  ...
  DMA_copy_x.number_of_2d_copies = 49;
  DMA_copy_x.number_of_1d_copies = 10;
  DMA_copy_x.length_1d_copy = 1;
  thorir_dma(&DMA_copy_x);
  pi_cl_team_barrier(0);

  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 22006) + 0;
  DMA_copy_W.number_of_2d_copies = 172;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_x = !db_state_x ? 490 : 0;
    db_W = !db_state_W ? 6880 : 0;
    db_y = !db_state_y ? 21500 : 0;
    db_act = !db_state_W ? 688 : 0;
    exec_db_x = 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 6880 : 0;
    exec_db_act = db_state_W ? 688 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      W_tile_size_nof = (_i_nof_load+1 == 1) ? 172 : 172;
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 172, 10*4, 1, 10*4, 1, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 22006) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        thorir_dma(&DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+6880 + 688*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 28894 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        thorir_dma(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+7568 + 688*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 29590 + db_act;
    ...
    k = (int32_t *) (l1_buffer + 28894 + exec_db_act);
    lambda = (int32_t *) (l1_buffer + 29590 + exec_db_act);
    W = (uint8_t *) (l1_buffer + 22006 + exec_db_W);
    y = (uint8_t *) (l1_buffer + 498 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 1 : 1;
    x_tile_size_h_exec   = (_i_h_exec+1 == 1)   ? 49 : 49;
    x_tile_size_w_exec   = (_i_w_exec+1 == 1)   ? 10 : 10;
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 172 : 172;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 1)   ? 172 : 172;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
   ...
   if(iter < (total_tiles-1) && (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec))
   {
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 25, 5, 172, 5, 172, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 498) + db_y;
