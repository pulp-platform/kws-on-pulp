...
void BNReluConvolution5(
  ...
  ////////////////////////////
  DMA_copy_k.ext = (uint32_t) l2_W+576;
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+832;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 16864;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+576 + 256*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 16600 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        thorir_dma(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+832 + 256*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 16864 + db_act;
    ...
    k = (int32_t *) (l1_buffer + 16600 + exec_db_act);
    lambda = (int32_t *) (l1_buffer + 16864 + exec_db_act);
