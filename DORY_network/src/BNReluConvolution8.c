...
void BNReluConvolution8(
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 25, 5, 64, 5, 64,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+4096 + 512*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 20120;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        thorir_dma(&DMA_copy_k);
        pi_cl_team_barrier(0);

        DMA_copy_lambda.ext = (uint32_t) l2_W+4608 + 512*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 20640;
    ...
    k = (int64_t *) (l1_buffer + 20120);
    lambda = (int64_t *) (l1_buffer + 20640);
