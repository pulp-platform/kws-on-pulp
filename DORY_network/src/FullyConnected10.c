...
void FullyConnected10(
  ...
  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 128) + 0;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_x = !db_state_x ? 64 : 0;
    db_W = !db_state_W ? 768 : 0;
    db_y = !db_state_y ? 48 : 0;
    exec_db_x = 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 768 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 12, 1*1, 64, 1*1, 64, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 128) + db_W;
   ...
   // wait for DMA write/read
     pi_cl_team_barrier(0);
      ...
      DMA_copy_y.loc = (l1_buffer + 72) + db_y;
