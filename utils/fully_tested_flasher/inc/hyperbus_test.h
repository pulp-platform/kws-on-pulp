...
static inline void udma_hyperflash_wwrite(unsigned int ext_addr, short int data, unsigned int tran_id){
  ...
  //Want to Char: abcdefgh
  //Cast to short int 00000000abcdefgh
  //Cast to int in rtl: 000000000000000000000000abcdefgh -> Good reading alignment
...
static inline void udma_hyperflash_bwrite(unsigned int nb_word, unsigned int hyper_waddr, unsigned int l2_addr, unsigned int tran_id){
    ...
    for(int i=0; i< nb_word; i++ ){
        ...
        // udma_hyperflash_wwrite(hyper_waddr+i, *((char *) l2_addr+i), tran_id); //Tested via prints
        // printf("LOOOL: %x\n", *((char *) l2_addr+i)); // confirming tests
    }
