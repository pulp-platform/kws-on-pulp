...
void layerConvBNRelu2(
  ...
  if(rt_core_id()==0){
    ...
    48,// length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
    32, // length_0: legnth of the 1_d copy, the length of tile in w direction
    1, // dir
    &dma_read_evt_x // copy
    );
    //printf("before 2nd dory_dma_memcpy_3d_custom ");
    dory_dma_memcpy_3d_custom(
    l2_W, // ext
    (l1_buffer + 36872) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
...
//     mchan_transfer(2048, 1, 1, 0, 1, 0, 0, (unsigned int)(l2_W), (unsigned int)((l1_buffer + 36872) + 0), 0, 0);
// % else:
//     dory_dma_memcpy_3d_custom(
//     l2_W, // ext
//     (l1_buffer + 36872) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
...
// % endif
   // wait for x,W read
    ...
    //printf("before 2nd rt_dma_memcpy ");
    rt_dma_memcpy(
    l2_W+2176, // ext
    l1_buffer + 41112, // loc
    256, // size
  ...
  // last-tile flags
    ...
    int last_w_load = (16 == 1) ? 1 : 0;

    int iter;
  // tile loop nest
      for(iter=0; iter<1*1*2*16; iter++) {
  if(rt_core_id()==0){
    ...
    last_w_load = (_i_w_load+1 == 16) ? 1 : 0;

    // compute double buffering offsets and update db state
    db_x = !db_state_x ? 6144 : 0;
    db_W = !db_state_W ? 2048 : 0;
    db_y = !db_state_y ? 12288 : 0;
    exec_db_x = db_state_x ? 6144 : 0;
    ...
    if(iter<1*1*2*16-1) {
     ...
     dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 48, 4, 32, 64, 32,  0, 0,0, pad_offset_h, pad_offset_w, 0, 1), // extern
      ...
      y_tile_size_h   = (last_h_load)   ? 16 : 48;
      ...
      /* if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
        dory_dma_memcpy_3d_custom(
          dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 64, 1*1, 32, 1*1, 32, 0,0,0,0,0,0, 1), // ext
          (l1_buffer + 36872) + db_W, // loc
...
// % if flag_DW == 0:
//          dma_read_evt_W = mchan_alloc();
//         mchan_transfer(W_tile_size_byte, 1, 1, 0, 1, 0, 0, (unsigned int)(dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 64, 1*1, 32, 1*1, 32, 0,0,0,0,0,0, 1)), (unsigned int)((l1_buffer + 36872) + db_W), 0, 0);
// % else:
//         dory_dma_memcpy_3d_custom(
//           dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 64, 1*1, 32, 1*1, 32, 0,0,0,0,0,0, 1), // ext
//           (l1_buffer + 36872) + db_W, // loc
//           W_tile_size_byte, // size: dimension of matrix of weight * bytes_per_weight
//           32, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
//           32, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
...
// % endif
    ...
    k = (int16_t *) (l1_buffer + 40980 + _i_nof_exec*128);
    lambda = (int32_t *) (l1_buffer + 41112 + _i_nof_exec*256);
    W = (char *) (l1_buffer + 36872 + exec_db_W);
    y = (char *) (l1_buffer + 12292 + db_y);
    x_tile_size_nif_exec = (last_nif_exec) ? 32 : 32;
    x_tile_size_h_exec   = (last_h_exec)   ? 16 : 48;
    x_tile_size_w_exec   = (last_w_exec)   ? 4 : 4;

    y_tile_size_nof = (last_nof_exec) ? 64 : 64;
    y_tile_size_h   = (last_h_exec)   ? 16 : 48;
    y_tile_size_w   = (last_w_exec)   ? 4 : 4;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*1;
    y_length_h_px = (last_h_exec) ? 16 : 48;
...
if(rt_core_id()==0){
    // wait for DMA write
      if(iter) {
       ...
       dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 48, 4, 64, 64, 64, 0, 0, 0, 0, 0, 0, 1), // ext
        (l1_buffer + 12292) + db_y, // loc
