...
// y_length_h_px                  8
// y_length_w_byte                4
// y_tile_size_nof_last           56
// y_tile_size_h_last             8
// y_tile_size_w_last             4
// y_length_nof_px_last           56
// y_length_w_byte_last           4
// y_length_nof_byte_last         56
...
// % endif
  ...
  // last-tile flags
    ...
    int last_w_load = (4 == 1) ? 1 : 0;

    int iter;
  // tile loop nest
  for(iter=0; iter<3*1*2*4; iter++) {
  if(rt_core_id()==0){
    ...
    if(iter<3*1*2*4-1) {
      ...
      dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 4, 128, 16, 128,  0, 0,0, pad_offset_h, pad_offset_w, 0, 1), // extern
      ...
      W_tile_size_nof = (last_nof_load) ? 56 : 100;
...
// % endif
    ...
    lambda = (int32_t *) (l1_buffer + 40728 + _i_nof_exec*400);
    ...
    y_tile_size_nof = (last_nof_exec) ? 56 : 100;
    ...
    y_length_nof_byte = (last_nof_exec)   ? 56 : 100;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
...
if(rt_core_id()==0){
    // wait for DMA write
      ...
      dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 4, 100, 16, 256, 0, 0, 0, 0, 0, 0, 1), // ext
