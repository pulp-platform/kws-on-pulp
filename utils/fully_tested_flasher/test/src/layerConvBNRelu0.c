...
void layerConvBNRelu0(
  ...
  if(rt_core_id()==0){
    im2col = l1_buffer + 42198;
    // copy first tiles
    //l2_x has now activations, input activations, accumulated activations over channels
    dory_dma_memcpy_3d_custom(
    l2_x, // ext
    (l1_buffer + 0) + 0, // loc
    5763, // size: dimension of the buffer
    384, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
    3, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
    113,// length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
    3, // length_0: legnth of the 1_d copy, the length of tile in w direction
    1, // dir
    &dma_read_evt_x // copy
    );
    dory_dma_memcpy_3d_custom(
    l2_W, // ext
    (l1_buffer + 40206) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
    864, // size: dimension of matrix of weight * bytes_per_weight
    27, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
    3, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
...
//     mchan_transfer(864, 1, 1, 0, 1, 0, 0, (unsigned int)(l2_W), (unsigned int)((l1_buffer + 40206) + 0), 0, 0);
// % else:
//     dory_dma_memcpy_3d_custom(
//     l2_W, // ext
//     (l1_buffer + 40206) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
...
// % endif
  ...
  if(rt_core_id()==0){
    ...
    // compute double buffering offsets and update db state
    db_x = !db_state_x ? 5763 : 0;
    db_W = !db_state_W ? 864 : 0;
    db_y = !db_state_y ? 14336 : 0;
    exec_db_x = db_state_x ? 5763 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 864 : 0;
    ...
    if(iter<1*1*2*8-1) {
      x_tile_size_nif = (last_nif_load) ? 3 : 3;
      x_tile_size_h   = (last_h_load)   ? 17 : 113;
      x_tile_size_w   = (last_w_load)   ? 17 : 17;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*1;
      x_length_h_px = (last_h_load) ? 17 : 113;
      x_length_nif_byte = (last_nif_load)   ? 3 : 3;
      ...
      dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 113, 17, 3, 128, 3,  1, 1,0, pad_offset_h, pad_offset_w, 0, 1), // extern
      ...
      y_tile_size_h   = (last_h_load)   ? 8 : 56;
      y_tile_size_w   = (last_w_load)   ? 8 : 8;
      W_tile_size_nof = (last_nof_load) ? 32 : 32;
      W_tile_size_nif = (last_nif_load) ? 3 : 3;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*1*3*3;
      W_length_nif_byte = (last_nif_load) ? 3 : 3;

      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
        dory_dma_memcpy_3d_custom(
          dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 32, 3*3, 3, 3*3, 3, 0,0,0,0,0,0, 1), // ext
          (l1_buffer + 40206) + db_W, // loc
...
// % if flag_DW == 0:
//          dma_read_evt_W = mchan_alloc();
//         mchan_transfer(W_tile_size_byte, 1, 1, 0, 1, 0, 0, (unsigned int)(dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 32, 3*3, 3, 3*3, 3, 0,0,0,0,0,0, 1)), (unsigned int)((l1_buffer + 40206) + db_W), 0, 0);
// % else:
//         dory_dma_memcpy_3d_custom(
//           dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 32, 3*3, 3, 3*3, 3, 0,0,0,0,0,0, 1), // ext
//           (l1_buffer + 40206) + db_W, // loc
//           W_tile_size_byte, // size: dimension of matrix of weight * bytes_per_weight
//           27, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
//           3, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
...
// % endif
    ...
    k = (int16_t *) (l1_buffer + 41946 + _i_nof_exec*64);
    lambda = (int32_t *) (l1_buffer + 42014 + _i_nof_exec*128);
    W = (char *) (l1_buffer + 40206 + exec_db_W);
    y = (char *) (l1_buffer + 11530 + db_y);
    x_tile_size_nif_exec = (last_nif_exec) ? 3 : 3;
    x_tile_size_h_exec   = (last_h_exec)   ? 17 : 113;
    x_tile_size_w_exec   = (last_w_exec)   ? 17 : 17;

    y_tile_size_nof = (last_nof_exec) ? 32 : 32;
    y_tile_size_h   = (last_h_exec)   ? 8 : 56;
    y_tile_size_w   = (last_w_exec)   ? 8 : 8;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*1;
    y_length_h_px = (last_h_exec) ? 8 : 56;
...
if(rt_core_id()==0){
    // wait for DMA write
      ...
      dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 56, 8, 32, 64, 32, 0, 0, 0, 0, 0, 0, 1), // ext
        (l1_buffer + 11530) + db_y, // loc
