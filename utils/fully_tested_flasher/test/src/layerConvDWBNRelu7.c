...
void layerConvDWBNRelu7(
  ...
  if(rt_core_id()==0){
    ...
    16, // length_0: legnth of the 1_d copy, the length of tile in w direction
    1, // dir
    &dma_read_evt_x // copy
    );
    dory_dma_memcpy_3d_custom(
    ...
    144, // size: dimension of matrix of weight * bytes_per_weight
...
//     mchan_transfer(144, 1, 1, 0, 1, 0, 0, (unsigned int)(l2_W), (unsigned int)((l1_buffer + 40968) + 0), 0, 0);
// % else:
//     dory_dma_memcpy_3d_custom(
//     l2_W, // ext
//     (l1_buffer + 40968) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
...
// % endif
  ...
  if(rt_core_id()==0){
    ...
    db_W = !db_state_W ? 144 : 0;
    db_y = !db_state_y ? 4096 : 0;
    exec_db_x = db_state_x ? 16384 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 144 : 0;
    ...
    if(iter<8*1*1-1) {
      x_tile_size_nif = (last_nif_load) ? 16 : 16;
      ...
      x_length_nif_byte = (last_nif_load)   ? 16 : 16;
      ...
      dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 32, 32, 16, 32, 128,  1, 1,0, pad_offset_h, pad_offset_w, 0, 1), // extern
      ...
      W_tile_size_nof = (last_nof_load) ? 16 : 16;
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
        dory_dma_memcpy_3d_custom(
          dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 16, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 1), // ext
...
// % if flag_DW == 0:
//          dma_read_evt_W = mchan_alloc();
//         mchan_transfer(W_tile_size_byte, 1, 1, 0, 1, 0, 0, (unsigned int)(dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 16, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 1)), (unsigned int)((l1_buffer + 40968) + db_W), 0, 0);
// % else:
//         dory_dma_memcpy_3d_custom(
//           dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 16, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 1), // ext
//           (l1_buffer + 40968) + db_W, // loc
...
// % endif
    ...
    k = (int16_t *) (l1_buffer + 41268 + _i_nof_exec*32);
    lambda = (int32_t *) (l1_buffer + 41528 + _i_nof_exec*64);
    W = (char *) (l1_buffer + 40968 + exec_db_W);
    y = (char *) (l1_buffer + 32772 + db_y);
    x_tile_size_nif_exec = (last_nif_exec) ? 16 : 16;
    x_tile_size_h_exec   = (last_h_exec)   ? 32 : 32;
    x_tile_size_w_exec   = (last_w_exec)   ? 32 : 32;

    y_tile_size_nof = (last_nof_exec) ? 16 : 16;
    ...
    y_length_nof_byte = (last_nof_exec)   ? 16 : 16;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
...
if(rt_core_id()==0){
    // wait for DMA write
      ...
      dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 16, 16, 16, 16, 128, 0, 0, 0, 0, 0, 0, 1), // ext
        (l1_buffer + 32772) + db_y, // loc
