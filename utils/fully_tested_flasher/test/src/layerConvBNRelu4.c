...
void layerConvBNRelu4(
  ...
  if(rt_core_id()==0){
    ...
    dory_dma_memcpy_3d_custom(
    l2_W, // ext
    (l1_buffer + 24584) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
    8192, // size: dimension of matrix of weight * bytes_per_weight
    64, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
    64, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
    128, // length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
    64, // length_0: legnth of the 1_d copy, the length of tile in w direction
    1, // dir
    &dma_read_evt_W // copy
    );
...
//     mchan_transfer(8192, 1, 1, 0, 1, 0, 0, (unsigned int)(l2_W), (unsigned int)((l1_buffer + 24584) + 0), 0, 0);
// % else:
//     dory_dma_memcpy_3d_custom(
//     l2_W, // ext
//     (l1_buffer + 24584) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
...
// % endif
  ...
  if(rt_core_id()==0){
    ...
    if(iter<1*1*2*8-1) {
      ...
      dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 16, 4, 64, 32, 64,  0, 0,0, pad_offset_h, pad_offset_w, 0, 1), // extern
      ...
      W_tile_size_nof = (last_nof_load) ? 128 : 128;
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
        dory_dma_memcpy_3d_custom(
          dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 128, 1*1, 64, 1*1, 64, 0,0,0,0,0,0, 1), // ext
          (l1_buffer + 24584) + db_W, // loc
...
// % if flag_DW == 0:
//          dma_read_evt_W = mchan_alloc();
//         mchan_transfer(W_tile_size_byte, 1, 1, 0, 1, 0, 0, (unsigned int)(dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 128, 1*1, 64, 1*1, 64, 0,0,0,0,0,0, 1)), (unsigned int)((l1_buffer + 24584) + db_W), 0, 0);
// % else:
//         dory_dma_memcpy_3d_custom(
//           dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 128, 1*1, 64, 1*1, 64, 0,0,0,0,0,0, 1), // ext
//           (l1_buffer + 24584) + db_W, // loc
//           W_tile_size_byte, // size: dimension of matrix of weight * bytes_per_weight
//           64, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
//           64, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
...
// % endif
    ...
    k = (int16_t *) (l1_buffer + 40980 + _i_nof_exec*256);
    lambda = (int32_t *) (l1_buffer + 41240 + _i_nof_exec*512);
    W = (char *) (l1_buffer + 24584 + exec_db_W);
...
if(rt_core_id()==0){
    // wait for DMA write
      ...
      dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 16, 4, 128, 32, 128, 0, 0, 0, 0, 0, 0, 1), // ext
