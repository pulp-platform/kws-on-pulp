...
//     mchan_transfer(36, 1, 1, 0, 1, 0, 0, (unsigned int)(l2_W), (unsigned int)((l1_buffer + 40968) + 0), 0, 0);
// % else:
//     dory_dma_memcpy_3d_custom(
//     l2_W, // ext
//     (l1_buffer + 40968) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
...
// % endif
  ...
  if(rt_core_id()==0){
    ...
    if(iter<16*1*1-1) {
      ...
      dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 64, 64, 4, 64, 64,  1, 1,0, pad_offset_h, pad_offset_w, 0, 1), // extern
      ...
      W_tile_size_nof = (last_nof_load) ? 4 : 4;
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
        dory_dma_memcpy_3d_custom(
          dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 4, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 1), // ext
...
// % if flag_DW == 0:
//          dma_read_evt_W = mchan_alloc();
//         mchan_transfer(W_tile_size_byte, 1, 1, 0, 1, 0, 0, (unsigned int)(dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 4, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 1)), (unsigned int)((l1_buffer + 40968) + db_W), 0, 0);
// % else:
//         dory_dma_memcpy_3d_custom(
//           dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 4, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 1), // ext
//           (l1_buffer + 40968) + db_W, // loc
...
// % endif
    ...
    k = (int16_t *) (l1_buffer + 41052 + _i_nof_exec*8);
    lambda = (int32_t *) (l1_buffer + 41184 + _i_nof_exec*16);
...
if(rt_core_id()==0){
    // wait for DMA write
      ...
      dory_dma_memcpy_3d_custom(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 32, 32, 4, 32, 64, 0, 0, 0, 0, 0, 0, 1), // ext
